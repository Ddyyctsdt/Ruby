...
class Crawler:
    ...
    def _scan_clickables(self, url: str, layer: int, clickable_id: int = 0) -> Optional[List[Dict]]:
        ...
        try:
            ...
            self.total_errors += 1
            self.failed_ops += 1
            error_title = f"❌ خطا در راند ۱ لایه {layer} (المان #{clickable_id})"
            error_detail = f"🌐 {url}\n━━━━━━━━━━━━━━━━━━━━━━\n{e}"
            self._log_error(f"{error_title}\n{error_detail}")
            self._invoke_callback(f"{error_title}\n```\n{error_detail}\n```")
            return None
        finally:
            if page:
                page.close()
    ...
    def _scan_downloads(self, url: str, layer: int, clickable_id: int = 0):
        ...
        try:
            ...
            self.total_errors += 1
            self.failed_ops += 1
            error_title = f"❌ خطا در راند ۲ لایه {layer} (المان #{clickable_id})"
            error_detail = f"🌐 {url}\n━━━━━━━━━━━━━━━━━━━━━━\n{e}"
            self._log_error(f"{error_title}\n{error_detail}")
            self._invoke_callback(f"{error_title}\n```\n{error_detail}\n```")
        finally:
